...
describe('ReviewService', () => {
  ...
  describe('createReview', () => {
    ...
    it('should create a review successfully', async () => {
      ...
      jest.spyOn(pharmacyService, 'getPharmacyById').mockResolvedValue(mockPharmacy);
    ...
    it('should throw BadRequestException if userId is not provided', async () => {
      await expect(service.createReview('', { pharmacyId: 'test', rating: 5 }))
        .rejects
    ...
    it('should throw NotFoundException if user is not found', async () => {
      ...
      await expect(service.createReview(mockUserId, { pharmacyId: 'test', rating: 5 }))
        .rejects
  ...
  describe('getPharmacyReviews', () => {
    it('should return reviews for a specific pharmacy', async () => {
      ...
      expect(result).toEqual(mockReviews.filter(review => review.pharmacyId === 'pharmacy123'));
  ...
  describe('getUserReviews', () => {
    it('should return reviews for a specific user', async () => {
      ...
      expect(reviewRepository.findAll).toHaveBeenCalled();
      expect(result).toEqual(mockReviews.filter(review => review.userId === 'user123'));
      expect(result).toHaveLength(2);
  ...
  describe('deleteReview', () => {
    it('should delete a review successfully', async () => {
      ...
      await expect(service.deleteReview('user123', 'review123')).resolves.not.toThrow();
    });
    ...
    it('should throw NotFoundException if review is not found', async () => {
      ...
      await expect(service.deleteReview('user123', 'review123'))
        .rejects
        .toThrow(NotFoundException);
    });
